...
(function (window, document) {
  ...
  function initSiteInteractions() {
    ...
    const contactSubmitLabel = contactSubmit?.querySelector(
      ".contact__submit-label",
    );
    const defaultSubmitLabel =
      contactSubmit?.dataset.defaultLabel ||
      contactSubmitLabel?.textContent ||
    ...
    const enhanceButtonLabel = (label) => {
      ...
      Array.from(text).forEach((character, index) => {
        ...
        span.className =
          character === " " ? "btn__char btn__char--space" : "btn__char";
    ...
    const enhanceButtonLabels = (root = document) => {
      const labels =
        root instanceof Element && root.matches(".btn__label")
          ? [root]
    ...
    if (heroSection) {
      window.addEventListener("scroll", updateNavVisualState, {
        passive: true,
      });
    ...
    if (reduced) {
      document
        .querySelectorAll(".hero__video, .contact__video")
        .forEach((video) => {
          video.pause();
  ...
  if (
    ...
    // ทำงานเฉพาะตอนที่ไม่ได้อยู่ใน Edit Mode เท่านั้น
