(function (window, document) {
  ...
  if (
    ...
    !document.documentElement.classList.contains("ccm-edit-mode")
  ) {
    ...
    function initHomePageInteractions() {
      ...
      if (document.querySelector(".about")) {
        ...
        aboutCharactersTl
          .to(
            ".about__cloud--1",
            { left: "-170px", autoAlpha: 1, duration: 0.8 },
            0,
          )
          .to(
            ".about__cloud--2",
            { right: "-170px", autoAlpha: 1, duration: 0.8 },
            0.05,
          )
      ...
      const workCounterCurrent = document.querySelector(
      ...
      if (workCards.length) {
        const setWorkDesktopHeight = () => {
          if (window.innerWidth <= 680) {
            workCards.forEach((card) => {
              ...
              card
                .querySelectorAll(
                  ".work__image, .work__content, .work__bg-pattern",
                )
                .forEach((el) => {
                  el.style.height = "";
                  el.style.minHeight = "";
          ...
          const measuredHeight = Math.max(
            ...
            ...workCards.map((card) => {
              const imageHeight =
                card.querySelector(".work__image")?.offsetHeight || 0;
              const contentHeight =
                card.querySelector(".work__content-inner")?.scrollHeight || 0;
        ...
        const syncWorkState = (activeIndex) => {
          ...
          if (workCounterCurrent) {
            workCounterCurrent.textContent = String(clampedIndex + 1).padStart(
              2,
              "0",
            );
          }
        ...
        if (workCounterTotal) {
          workCounterTotal.textContent = String(workCards.length).padStart(
            2,
            "0",
          );
        }
      ...
      if (
        ...
        const CLIP_IN_HIDDEN =
          "polygon(0% 118%, 100% 100%, 100% 100%, 0% 100%)";
        ...
        const srvTL = gsap.timeline({
          scrollTrigger: {
            ...
            onUpdate(self) {
              const idx = Math.min(
                SRV_COUNT - 1,
                Math.max(0, Math.round(self.progress * SRV_COUNT) - 1),
              );
              setActiveNav(idx);
            },
          },
        ...
        for (let i = 1; i < SRV_COUNT; i += 1) {
          ...
          srvTL.fromTo(
            sImgs[i],
            { clipPath: CLIP_IN_HIDDEN },
            { clipPath: CLIP_IN_FULL, ease: "none", duration: 1 },
            at,
          );
          srvTL.fromTo(
            sImgs[i - 1],
            { clipPath: CLIP_OUT_FULL },
            { clipPath: CLIP_OUT_GONE, ease: "none", duration: 1 },
            at,
          );
          srvTL.to(
            sTexts[i - 1],
            { yPercent: -110, ease: "none", duration: 1 },
            at,
          );
        ...
        srvTL.to(
          navHead,
          { y: -(headBottom + 24), ease: "none", duration: 0.5 },
          0,
        );
        ...
        navLinks.forEach((link, index) => {
          link.addEventListener("click", (event) => {
            ...
            const y =
              st.start + (st.end - st.start) * ((index + 1) / SRV_COUNT);
    ...
    onWindowLoad(initHomePageInteractions);
  }
})(window, document);
