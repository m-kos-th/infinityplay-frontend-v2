Concrete.event.bind('btBlockWorkFeatured.workList.edit.open', function (options, settings) {
    ...
    var ftImageLogo = $(newField).find('.ft-image-workList-logo-file-selector');
            if ($(ftImageLogo).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageLogo).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorLogo = $(newField).find('.ft-image-workList-logo-page-selector');
            if ($(pageSelectorLogo).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorLogo).attr('id'),
                        components: config.components
                    });
                });
            }
    var ftFileVideoGamePreview = $(newField).find('.ft-file-workList-videoGamePreview-file-selector');
            if ($(ftFileVideoGamePreview).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftFileVideoGamePreview).attr('id'),
                        components: config.components
                    });
                });
            }
    blockDesignerEditor('#' + ($(newField).find('textarea.ft-workList-content').attr('id')).replace(/(:|\.|\[|\]|,|=|@)/g, "\\$1"));
        $.each($(newField).find('.ft-smart-link'), function () {
            var ftSmartLinkImage = $(this).find('.ft-smart-link-workList-button-image-selector');
            if ($(ftSmartLinkImage).length > 0) {
                 Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftSmartLinkImage).attr('id'),
                        components: config.components
                    });
                 });
            }
            var ftSmartLinkFile = $(this).find('.ft-smart-link-workList-button-file-selector');
            if ($(ftSmartLinkFile).length > 0) {
                 Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftSmartLinkFile).attr('id'),
                        components: config.components
                    });
                 });
            }
            var ftSmartLinkPage = $(this).find('.ft-smart-link-workList-button-page-selector');
            if ($(ftSmartLinkPage).length > 0) {
                 Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftSmartLinkPage).attr('id'),
                        components: config.components
                    });
                 });
            }
        ...
        if (deleteIt === true) {
            ...
            $(sortableItem).slideUp(slideSpeed, function () {
                if(typeof CKEDITOR !== 'undefined'){
                    var blockDesignerEditorInstance = CKEDITOR.instances[$(sortableItem).find('textarea.ft-workList-content').attr('id')];
