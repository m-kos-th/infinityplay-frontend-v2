Concrete.event.bind('btBlockServicesHome.servicesList.edit.open', function (options, settings) {
    ...
    var ftImageImagePreview = $(newField).find('.ft-image-servicesList-imagePreview-file-selector');
            if ($(ftImageImagePreview).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageImagePreview).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorImagePreview = $(newField).find('.ft-image-servicesList-imagePreview-page-selector');
            if ($(pageSelectorImagePreview).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorImagePreview).attr('id'),
                        components: config.components
                    });
                });
            }
    var ftFileVideoPreview = $(newField).find('.ft-file-servicesList-videoPreview-file-selector');
            if ($(ftFileVideoPreview).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftFileVideoPreview).attr('id'),
                        components: config.components
                    });
                });
            }
