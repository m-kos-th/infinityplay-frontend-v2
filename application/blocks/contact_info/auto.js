Concrete.event.bind('btContactInfo.contactItems.edit.open', function (options, settings) {
    ...
    blockDesignerEditor('#' + ($(newField).find('textarea.ft-contactItems-value').attr('id')).replace(/(:|\.|\[|\]|,|=|@)/g, "\\$1"));
    };
        ...
        if (deleteIt === true) {
            ...
            $(sortableItem).slideUp(slideSpeed, function () {
                if(typeof CKEDITOR !== 'undefined'){
                    var blockDesignerEditorInstance = CKEDITOR.instances[$(sortableItem).find('textarea.ft-contactItems-value').attr('id')];
